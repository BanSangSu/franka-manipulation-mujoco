...
def knn_points(X: torch.Tensor, K: int, norm: int):
    ...
    # Compute in batches to avoid OOM for large N
    batch_size = 2000
    all_dists = []
    all_idxs = []
    ...
    for i in range(0, N, batch_size):
        ...
        # Compute pairwise distances from batch to all points
        dist_matrix = torch.cdist(X_batch, X, p=norm)  # (B, N)

        # Ignore self-distance
        batch_len = end_idx - i
        row_idx = torch.arange(batch_len, device=X.device)
        dist_matrix[row_idx, row_idx + i] = float("inf")

        # Get the indices of the K-nearest neighbors
        k_capped = min(K, N - 1) if N > 1 else 1 # Fallback for extremely small point clouds
        if N <= 1: # Degenerate case
            all_dists.append(torch.zeros((end_idx-i, 1), device=X.device))
            all_idxs.append(torch.zeros((end_idx-i, 1), dtype=torch.long, device=X.device))
            continue
            
        batch_dists, batch_idxs = torch.topk(dist_matrix, k_capped, dim=1, largest=False)
