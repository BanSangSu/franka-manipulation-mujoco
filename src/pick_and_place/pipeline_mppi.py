...
class PickAndPlacePipeline:
    ...
    def run(self) -> PipelineResult:
        ...
        # ---- CONTROL: Move to grasp (MPPI with obstacle avoidance) ----
        logger.info("═══ Stage 3: CONTROL (MPPI → Grasp) ═══")

        # Start the obstacle tracker thread (like baseline code pattern)
        tracker = self.motion_planner._obstacle_tracker
        if tracker is not None:
            logger.info("Starting obstacle tracker thread...")
            tracker.start()

        logger.info("Executing trajectory with MPPI + real-time obstacle avoidance...")

        exec_result = self.motion_planner.execute(traj)
        ...
        if result.grasp_success:
            ...
            logger.warning(
                "✗ MPPI execution did not fully converge (error=%.4f)",
                exec_result.get("final_error", float("inf")),
            )
            # Stop tracker thread before returning
            if tracker is not None:
                tracker.stop()
            return result
        ...
        # ---- PLACE: Find basket and move there ----
