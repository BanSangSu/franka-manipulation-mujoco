"""
MotionPlanner – orchestrates IK, RRT-Connect (global path), and Artificial Potential Field/MPPI (local control).
...
class MotionPlanner:
    ...
    def __init__(
        ...
        self._welded_object_gravcomp: Optional[float] = None

    def _get_target_body_name(self) -> Optional[str]:
        """Return the MuJoCo body name for the configured grasp target."""
        grasp_meta = self.sim.ids.get("grasp_object", {})
        if isinstance(grasp_meta, dict):
            body_name = grasp_meta.get("body_name")
            if body_name:
                return str(body_name)
        return None

    def _get_ee_body_name(self) -> str:
        """Return the configured end-effector body name."""
        robot_cfg = dict(getattr(self.sim, "robot_settings", {}))
        mujoco_cfg = dict(getattr(self.sim, "cfg", {}).get("mujoco", {}))
        return str(
            robot_cfg.get(
                "ee_body_name",
                mujoco_cfg.get("ee_body_name", "hand"),
            )
        )

    def _sync_attached_body_collision_state(self):
        """Update the collision checker with the currently carried object."""
        if self._collision_checker is not None:
            self._collision_checker.set_attached_body_name(self._welded_object)

    def _linear_path_if_collision_free(
        self,
        q_start: np.ndarray,
        q_goal: np.ndarray,
        n_steps: int,
        ...
        """Return a linear joint-space fallback path only if it is collision-free."""
        if self._collision_checker(q_goal, allow_target_contact=allow_target_contact):
            return None
        if self._collision_checker.check_edge(
            q_start,
            q_goal,
            allow_target_contact=allow_target_contact,
        ):
            return None
        return self._linear_interpolate(q_start, q_goal, n_steps=n_steps)
    ...
    def _init_rrt(self):
        ...
        self._collision_checker = MuJoCoCollisionChecker(
            ...
            self_collision=True,
            env_collision=True,
            target_body_name=self._get_target_body_name(),
            ee_body_name=self._get_ee_body_name(),
        )
        self._sync_attached_body_collision_state()
    ...
    def _init_mppi(self):
        ...
        if self._obstacle_tracker is not None:
    ...
    def plan_to_pose(
        ...
        if path_to_pregrasp is None:
            ...
            path_to_pregrasp = self._linear_path_if_collision_free(
                start_q,
                pre_grasp_q,
                n_steps=30,
            )
            if path_to_pregrasp is None:
                logger.error("Linear fallback also collides: start → pre-grasp")
                return []
        ...
        if path_approach is None:
            logger.warning("RRT failed for approach phase, trying checked linear fallback")
            path_approach = self._linear_path_if_collision_free(
                pre_grasp_q,
                grasp_q,
                n_steps=15,
            ...
            if path_approach is None:
                logger.error("Linear fallback also collides: pre-grasp → grasp")
                return []
    ...
    def plan_to_joint_config(
        ...
        if path is None:
            ...
            path = self._linear_path_if_collision_free(
                start_q,
                target_q,
                n_steps=50,
            )
            if path is None:
                logger.error("Linear fallback also collides: current → target joint config")
                return []
    ...
    def plan_to_position(
        ...
        if path is None:
            ...
            if path is None:
                logger.error("Linear fallback also collides: current → target position")
                return []
    ...
    def execute_to_joint_goal_mppi(
        self,
        target_q: np.ndarray,
        max_steps: int = 2000,
    ) -> dict:
        """Execute MPPI directly toward a joint-space goal without RRT."""
        self._init_mppi()
        return self._mppi.execute_to_goal(
            sim=self.sim,
            goal_q=target_q,
            max_steps=max_steps,
            step_callback=self.weld_step,
    ...
    def execute_to_position_mppi(
        self,
        goal_pos: np.ndarray,
        goal_quat: Optional[np.ndarray] = None,
        max_steps: int = 2000,
    ) -> dict:
        """Solve IK for the goal, then let MPPI reach it directly."""
        target_q = self._solve_ik(goal_pos, goal_quat)
        if target_q is None:
            logger.error("IK failed for MPPI direct-goal execution")
            return {
                "success": False,
                "steps": 0,
        ...
        return self.execute_to_joint_goal_mppi(target_q, max_steps=max_steps)

    def execute_to_pose_mppi(
        self,
        goal_pos: np.ndarray,
        goal_quat: np.ndarray,
        max_steps: int = 2000,
    ) -> dict:
        """Pose-target wrapper for MPPI-only execution."""
        return self.execute_to_position_mppi(
            goal_pos=goal_pos,
            goal_quat=goal_quat,
            max_steps=max_steps,
        )
    ...
    def execute_apf(
        ...
        """Execute a local workspace motion using an APF planner."""
        from .apf_local_planner import APFLocalPlanner

        planner = APFLocalPlanner(
            model=self.model,
            data=self.data,
            robot=self.robot,
            tracker=self._obstacle_tracker,
            dt=self.mppi_dt,
            carried_body_name=self._welded_object,
        )
    ...
    def weld_object_to_ee(self, object_name: str):
        ...
        self._welded_object_gravcomp = float(self.model.body_gravcomp[obj_id])
    ...
    def release_welded_object(self):
        """Release the currently welded object so physics can place it."""
        if self._welded_object is None:
            return

        import mujoco

        obj_id = mujoco.mj_name2id(
            self.model, mujoco.mjtObj.mjOBJ_BODY, self._welded_object
        )
        if obj_id >= 0 and self._welded_object_gravcomp is not None:
            self.model.body_gravcomp[obj_id] = self._welded_object_gravcomp

        logger.info("Object '%s' released from end-effector.", self._welded_object)
        self._welded_object = None
        self._weld_rel_pos = None
        self._weld_rel_mat = None
        self._welded_object_gravcomp = None
        self._sync_attached_body_collision_state()
