...
class TrackingCost:
    ...
    def __call__(self, state: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
        ...
        # 1) Tracking cost
        # MPPI may evaluate the running cost many times while sampling. This
        # cost therefore must stay stateless; otherwise the reference index can
        # drift between rollouts and produce unstable behaviour.
        if self._ref is not None:
            diff = state.unsqueeze(1) - self._ref.unsqueeze(0)  # (K, T, nj)
            tracking_cost = (diff * diff).sum(dim=-1)  # (K, T)
            cost = cost + self.w_tracking * tracking_cost.min(dim=1).values

        # 2) Obstacle proximity cost
        #    Use the closest predicted obstacle location across the current
        #    horizon, which avoids relying on any mutable timestep counter.
        if self._fk_fn is not None and self._obstacle_traj is not None:
            ee_pos = self._fk_fn(state)  # (K, 3)
            horizon_obs = self._obstacle_traj  # (T, n_obs, 3)
            n_obs = horizon_obs.shape[1]

            for oi in range(n_obs):
                obs_pos = horizon_obs[:, oi, :]  # (T, 3)
                obs_r = self._obstacle_radii[oi] if self._obstacle_radii is not None else 0.06

                d = torch.norm(
                    ee_pos.unsqueeze(1) - obs_pos.unsqueeze(0),
                    dim=-1,
                ).min(dim=1).values  # (K,)
...
class MPPIController:
    ...
    def execute_to_goal(
        ...
        total_steps = 0
        logger.info("MPPI executing direct goal tracking (horizon=%d)...", T)

        while total_steps < max_steps:
            self._capture_tracker_frame()
            self._cost.set_reference(goal_slice)
            ...
            if goal_error < 0.05:
                logger.info(
                    "MPPI direct-goal ✓ reached goal! steps=%d error=%.4f",
                    total_steps,
                    goal_error,
                )
                return {
            ...
            if total_steps % 100 == 0:
                logger.info(
                    "MPPI direct-goal step %d: goal_err=%.4f",
                    total_steps,
                    goal_error,
                )
        ...
        success = final_error < 0.1
        logger.info(
            "MPPI direct-goal done: steps=%d error=%.4f success=%s",
            total_steps,
            final_error,
            success,
        )
